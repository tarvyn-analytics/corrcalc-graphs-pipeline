 ...
 * {@code CSV → ReturnPanels → S1 → S3 → alert} chain ({@link CryptoBacktest}) over the real saved
...
class CryptoFullBacktestDriverTest {
    ...
    void fullPipelineReproducesCryptoLeadTableRow(String name) {
        ...
        LeadTableRow row = CryptoBacktest.backtestEvent(dataDir, event, universe);
