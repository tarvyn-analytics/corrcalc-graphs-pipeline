...
class RearmCadenceTest {
    ...
    /** Builds the signal and observes it, handing the cadence the same density the signal carries
     * (this oracle never normalizes -- the density-unit compare is PipelineEnginePairUniverseTest's
     * job). */
    private static RearmCadence.Rearm advance(RearmCadence c, FireDirection dir, double sPlus, double density) {
        return c.observe(sig(dir, sPlus, density), density);
    }
    ...
    void intraday_MultiFusion_RearmsOnAllClearPlusCoolDown_FiresNTimes() {
        ...
        advance(c, FireDirection.FUSION, 9.0, 0.9);
        assertEquals(0, c.windowId(), "the fire bar feeds no trigger");
        advance(c, FireDirection.NONE, 1.0, 0.3);
        advance(c, FireDirection.NONE, 0.5, 0.3);
        advance(c, FireDirection.DEFUSION, 0.2, 0.3);
        assertEquals(0, c.windowId(), "the all-clear starts the cool-down, not the re-arm");
        advance(c, FireDirection.NONE, 0.1, 0.3);
        advance(c, FireDirection.NONE, 0.1, 0.3);
        ...
        advance(c, FireDirection.FUSION, 9.5, 0.9);
        advance(c, FireDirection.DEFUSION, 0.2, 0.3);
        advance(c, FireDirection.NONE, 0.1, 0.3);
        advance(c, FireDirection.NONE, 0.1, 0.3);
        advance(c, FireDirection.NONE, 0.1, 0.3);
    ...
    void intraday_CalmBetweenEvents_NoRearmNoClock() {
        ...
        advance(c, FireDirection.FUSION, 9.0, 0.9);
        advance(c, FireDirection.DEFUSION, 0.2, 0.3);   // cool-down 0: re-arms on the all-clear bar
    ...
    void daily_UnresolvedRegime_CalendarBackstopForcesRearm() {
        ...
        for (int i = 0; i < 20; i++) {
            advance(c, FireDirection.NONE, 5.0, 0.9);
        }
        assertEquals(0, c.windowId());
        advance(c, FireDirection.NONE, 5.0, 0.9);   // the 21st bar after the fire
    ...
    void observe_ReportsTheRearmKind_ResolvedVsExpired() {
        ...
        advance(resolved, FireDirection.FUSION, 9.0, 0.9);
        assertEquals(RearmCadence.Rearm.NONE, advance(resolved, FireDirection.NONE, 1.0, 0.9));
        assertEquals(RearmCadence.Rearm.RESOLVED, advance(resolved, FireDirection.DEFUSION, 0.2, 0.3),
                "cool-down 0: the all-clear bar re-arms as RESOLVED");

        RearmCadence expired = daily(3, 4);
        advance(expired, FireDirection.FUSION, 9.0, 0.9);
        for (int i = 0; i < 3; i++) {
            assertEquals(RearmCadence.Rearm.NONE, advance(expired, FireDirection.NONE, 5.0, 0.9));
        }
        assertEquals(RearmCadence.Rearm.EXPIRED, advance(expired, FireDirection.NONE, 5.0, 0.9),
                "the 4th elevated bar after the fire trips the backstop as EXPIRED");

        RearmCadence relaxed = daily(2, 0);
        advance(relaxed, FireDirection.FUSION, 9.0, 0.9);
        assertEquals(RearmCadence.Rearm.NONE, advance(relaxed, FireDirection.NONE, 1.0, 0.3));
        assertEquals(RearmCadence.Rearm.RESOLVED, advance(relaxed, FireDirection.NONE, 0.5, 0.3),
